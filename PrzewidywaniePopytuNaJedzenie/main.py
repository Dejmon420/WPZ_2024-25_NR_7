...
# Trenowanie modelu XGBoostRegressor
...
xgb_rmse = mean_squared_error(y_train, xgb_train_predictions, squared=False)

# Wizualizacja wynikóww RMSE dal każdego modelu
plt.figure(figsize=(10, 5))
plt.bar(['Random Forest', 'Gradient Boosting', 'XGBoostRegressor'], [rf_rmse, gb_rmse, xgb_rmse], color=['blue', 'green', 'purple'])
plt.ylabel('RMSE')
plt.title('Porównanie błędu RMSE dla Random Forest, Gradient Boosting i XGBoostRegressor')
plt.show()
...
plt.plot(xgb_predictions[:50], label='XGBoostRegressor Pzewidywanie', color='purple')
plt.legend()
